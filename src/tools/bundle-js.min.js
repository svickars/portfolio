...
},{"../helpers/typeof":3}],3:[function(require,module,exports){
...
function init() {
  ...
  d3.selectAll(".project .project-title").on("click", function () {
    var newHeight = d3.select(this.parentNode).classed("expanded") ? 0 : d3.select(this.parentNode).select(".inner").node().offsetHeight + 37;
    d3.select(this.parentNode).classed("expanded", !d3.select(this.parentNode).classed("expanded")).select(".inner-wrapper").style("height", "".concat(newHeight, "px"));
    d3.selectAll(".pass-container").classed("is-visible", false);
    d3.selectAll(".censored-project").select(".project-title-left").select("h2").classed("is-bg", false);
  ...
  d3.selectAll(".project .censored-project").on("click", function () {
    if (!d3.select(this.parentNode).classed("expanded")) {
      ...
      d3.select(this).select(".project-title-left").select("h2").classed("is-bg", true);
      d3.select(this).select(".pass-container").classed("is-visible", true);
      document.getElementById(id).focus();
      replaceUrl("");
    } else {
      d3.select(this.parentNode).classed("expanded", false).select(".inner-wrapper").style("height", 0);
      removeContent(d3.select(this).select(".pass-container").select("input").attr("id"));
      d3.selectAll(".project").classed("is-bg", function () {
        return !d3.select(this).classed("expanded");
      });
  ...
  d3.selectAll(".pass-container").select("input").on("keydown", function () {
    var _this = this;
    ...
    if (d3.event.keyCode == 13) {
      if (d3.select(this).node().value == id) {
        loadNewContent(id);
        setTimeout(function () {
          var newHeight = d3.select(_this.parentNode.parentNode.parentNode.parentNode).select(".inner").node().offsetHeight + 37;
          d3.select(_this.parentNode.parentNode.parentNode.parentNode).classed("expanded", true).select(".inner-wrapper").style("height", "".concat(newHeight, "px"));
          d3.select(_this.parentNode).classed("is-visible", false);
          d3.select(_this.parentNode.parentNode).select("h2").classed("is-bg", false);
          d3.selectAll(".project").classed("is-bg", function () {
            return !d3.select(this).classed("expanded");
          });
...
function resizeGeneral() {
  d3.selectAll(".project.expanded").select(".inner-wrapper").style("height", function () {
    var newHeight = d3.select(this).select(".inner").node().offsetHeight + 37;
    return "".concat(newHeight, "px");
  });
}

function loadCopy() {
  Promise.all([d3.json("./assets/data/copy.json")]).then(function (res) {
    copy = res[0];
  })["catch"](function (err) {
...
function censored() {
  d3.selectAll(".censored").style("padding", function () {
    return "0 ".concat(random(48, 16), "px 0 ").concat(random(48, 16), "px");
...
function loadNewContent(id) {
  var thisCopy = copy.work.find(function (d) {
    return d.id == id;
  }),
      container = d3.select("#".concat(id));
  container.select("h2").html(thisCopy.project0);

  if (thisCopy.cover0) {
    container.select(".cover").select("img").attr("src", "/assets/img/cover/".concat(thisCopy.cover0));
  }

  if (thisCopy.description0) {
    container.select(".description").html("");
    thisCopy.description0.forEach(function (d) {
      container.select(".description").append("p").html(d.value);
    });
  }

  if (thisCopy.links0) {
    container.select(".links").selectAll(".entry").each(function (d, i) {
  ...
  if (thisCopy.tags0) {
    container.select(".tags").selectAll(".entry").each(function (d, i) {
...
function scroll(id) {
  var target = (0, _jquery["default"])("#".concat(id));

  if (target.length) {
    var scrollTo = target.offset().top;
    (0, _jquery["default"])("body, html").animate({
      scrollTop: "".concat(scrollTo - 32, "px")
    }, 800);
  }
}
...
function goToContent() {
  ...
  if (url.includes("#")) {
    ...
    if (id !== "#" && id !== "") {
      scroll(id);
      var newHeight = d3.select("#".concat(id)).select(".inner-wrapper").select(".inner").node().offsetHeight + 37;
      d3.select("#".concat(id)).classed("expanded", true).select(".inner-wrapper").style("height", "".concat(newHeight, "px"));
      d3.selectAll(".project").classed("is-bg", function () {
        return !d3.select(this).classed("expanded");
      });
    }
...
function replaceUrl(id) {
  var url = window.location.href,
      old = url.split("#")[1]; // window.location = newUrl;
...
},{"@babel/runtime/helpers/interopRequireDefault":1,"@babel/runtime/helpers/interopRequireWildcard":2,"d3":35,"jquery":36}]},{},[37]);
